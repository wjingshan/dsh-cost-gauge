...
.dsg-bg{position:absolute;inset:0;z-index:0;border-radius:14px;background:rgba(20,22,28,.94);
  /* 挖洞 = 「横向补集」∪「纵向补集」（De Morgan），两侧各留 --dsg-feather 的渐变，避免生硬的直边 */
  -webkit-mask-image:linear-gradient(90deg,#000 0,#000 calc(var(--dsg-hole-x,0px) - var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-x,0px) + var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-x,0px) + var(--dsg-hole-w,0px) - var(--dsg-feather,8px)),#000 calc(var(--dsg-hole-x,0px) + var(--dsg-hole-w,0px) + var(--dsg-feather,8px)),#000 100%),linear-gradient(180deg,#000 0,#000 calc(var(--dsg-hole-y,0px) - var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-y,0px) + var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-y,0px) + var(--dsg-hole-h,0px) - var(--dsg-feather,8px)),#000 calc(var(--dsg-hole-y,0px) + var(--dsg-hole-h,0px) + var(--dsg-feather,8px)),#000 100%);
  mask-image:linear-gradient(90deg,#000 0,#000 calc(var(--dsg-hole-x,0px) - var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-x,0px) + var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-x,0px) + var(--dsg-hole-w,0px) - var(--dsg-feather,8px)),#000 calc(var(--dsg-hole-x,0px) + var(--dsg-hole-w,0px) + var(--dsg-feather,8px)),#000 100%),linear-gradient(180deg,#000 0,#000 calc(var(--dsg-hole-y,0px) - var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-y,0px) + var(--dsg-feather,8px)),rgba(0,0,0,0) calc(var(--dsg-hole-y,0px) + var(--dsg-hole-h,0px) - var(--dsg-feather,8px)),#000 calc(var(--dsg-hole-y,0px) + var(--dsg-hole-h,0px) + var(--dsg-feather,8px)),#000 100%);
...
`
        ...
        /** 过渡带宽度：随模糊半径走（模糊越强，边缘渐隐越宽，避免生硬直边）。 */
        const frostFeather = () => (frostBlur > 0 ? Math.min(18, Math.max(5, frostBlur * 0.85)) : 6)
        ...
        function applyFrostStyle() {
          ...
          root.style.setProperty('--dsg-feather', frostFeather().toFixed(1) + 'px')
        }
        ...
        function updateFrost() {
          ...
          const rr = root.getBoundingClientRect()
          // 标题栏（拖动把手）不参与磨砂：留着它做参照物，方便定位与拖动。
          const handleEl = root.querySelector(root.classList.contains('dsg-collapsed') ? '.dsg-mini' : '.dsg-title')
          // 标题栏下方再留出整段过渡带，避免渐隐渗到标题栏上（标题栏保持干净，方便定位/拖动）。
          const handleBottom = (handleEl ? Math.max(0, handleEl.getBoundingClientRect().bottom - rr.top + 2) : 0) + frostFeather()
          const x1 = Math.max(rr.left, chat.left)
          const x2 = Math.min(rr.right, chat.right)
          const y2 = Math.min(rr.bottom, chat.bottom)
          const y1 = Math.max(Math.max(rr.top, chat.top), rr.top + handleBottom)
          const iw = x2 - x1
          const ih = y2 - y1
          if (iw < 3 || ih < 8) {
            root.classList.remove('dsg-frosting')
          ...
          const hy = Math.max(handleBottom, y1 - rr.top + pad)
          const hw = Math.max(0, iw - pad * 2)
          const hh = Math.max(0, y2 - rr.top - pad - hy)
