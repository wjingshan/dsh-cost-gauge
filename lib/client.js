...
`
        ...
        function render(data) {
          ...
          // 档位徽章：按当前会话模型动态显示（pro → Pro，其余 → Basic）。
          const isPro = !!(cost && cost.pricingKey === 'pro')
          badgeEl.textContent = isPro ? 'Pro' : 'Basic'
          badgeEl.classList.toggle('dsg-badge-pro', isPro)
          badgeEl.title = isPro ? '当前会话使用 Pro 模型' : '当前会话使用基础（Flash）模型'
