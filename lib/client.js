...
.dsg-star{animation:dsg-twinkle 3s ease-in-out infinite}
@keyframes dsg-twinkle{0%,100%{opacity:.75}50%{opacity:.35}}
...
`
    ...
    function populateStars(layer, isWeekend, isPM, count = 60) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const c = svgEl('circle', { cx: x.toFixed(1), cy: y.toFixed(1), r: (0.45 + Math.random() * 0.8).toFixed(2), fill: color })
        c.setAttribute('class', 'dsg-star')
