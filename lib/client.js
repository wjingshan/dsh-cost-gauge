...
.dsg-model{flex:none}
.dsg-model-pill{display:inline-block;position:relative;overflow:hidden;perspective:80px;
  ...
  box-shadow:0 2px 6px rgba(0,0,0,.35);height:20px;box-sizing:border-box;min-width:46px;text-align:center}
.dsg-model-inner{display:block;line-height:14px;white-space:nowrap}
...
`
        ...
        function setModelText(text) {
          if (modelTextEl.dataset.val === text) return
          const old = modelTextEl.firstElementChild
          const nw = document.createElement('span')
          nw.className = 'dsg-model-inner' + (old ? ' dsg-model-in' : '')
          nw.textContent = text
          modelTextEl.appendChild(nw)
          modelTextEl.dataset.val = text
          if (old) {
