...
`
    ...
    /** 构建 12h 外圈：连续无缝圆环——黄底整圆 + 绿色空闲弧（圆头，与黄底圆角交接）；周末全绿。
     *  空闲/繁忙按 DeepSeek 实际时段划分：工作日繁忙 09:00–12:00、14:00–18:00，
     *  即 o'clock 9/10/11 与 2/3/4/5（黄）；空闲为 12/1 与 6/7/8（绿，含 18:00）。 */
    function buildClockRing(ringEl, isWeekend) {
      ...
      if (isWeekend) {
        ...
        c.setAttribute('stroke', COL.idleRing)
      ...
      base.setAttribute('stroke', COL.busyRing)
      ringEl.appendChild(base)
      // 绿色空闲弧：o'clock 12+1 → 345°..45°；6+7+8 → 165°..255°（18:00 属空闲）。
      const arcs = [
      ...
      for (const d of arcs) {
        ...
        p.setAttribute('stroke', COL.idleRing)
    ...
    function renderOdometer(odoEl, balance, threshold, maxBalance) {
      ...
      // 标签。
      const l0 = svgEl('text', { class: 'dsg-odo-label', x: '30', y: '106', 'text-anchor': 'start' })
      l0.textContent = '¥0'
      const l1 = svgEl('text', { class: 'dsg-odo-label', x: '170', y: '106', 'text-anchor': 'end' })
