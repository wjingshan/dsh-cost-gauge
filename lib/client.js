...
.dsg-star{animation:dsg-twinkle 2.2s ease-in-out infinite}
@keyframes dsg-twinkle{0%,100%{opacity:.1}50%{opacity:.95}}
...
`
    ...
    /** 星空皮肤：在空闲弧带内散布闪烁星星。 */
    function populateStars(layer, isWeekend, isPM, count = 40) {
      layer.textContent = ''
      ...
      for (let i = 0; i < count; i++) {
        ...
        const c = svgEl('circle', { class: 'dsg-star', cx: x.toFixed(1), cy: y.toFixed(1), r: (0.7 + Math.random() * 0.9).toFixed(2) })
        c.style.animationDelay = (Math.random() * 2.6).toFixed(2) + 's'
        c.style.animationDuration = (1.5 + Math.random() * 2).toFixed(2) + 's'
        c.setAttribute('fill', Math.random() < 0.25 ? '#a9c8f0' : '#dce8ff')
        layer.appendChild(c)
      }
    }
...
<div class="dsg-compact">
  ...
  <div class="dsg-row"><span>余额</span><span class="dsg-val dsg-balance2">—</span></div>
          ...
          function renderRing() {
            ...
            if (key !== lastRingKey) {
              lastRingKey = key
              const idleC = (stars && dark) ? STAR_BLUE : idle
