...
.dsg-hub{fill:#fff}
.dsg-odo-track{fill:none;stroke:rgba(255,255,255,.10);stroke-width:11;stroke-linecap:round}
.dsg-odo-red{fill:none;stroke:${COL.odoRed};stroke-width:11;stroke-linecap:round}
.dsg-odo-fuel{fill:none;stroke:${COL.odoGreen};stroke-width:11;stroke-linecap:round}
...
.dsg-hit-label{fill:#949494;font-size:8.5px;text-anchor:middle}
...
.dsg-pref-row label{font-size:11px;color:#949494;margin-bottom:0;display:block;flex:1}
.dsg-ring-w{width:56px;box-sizing:border-box;background:rgba(255,255,255,.08);
  border:1px solid rgba(255,255,255,.16);border-radius:6px;color:#fff;font-size:12px;padding:4px 6px}
.dsg-color{width:34px;height:24px;padding:0;border:1px solid rgba(255,255,255,.2);
...
`
     ...
     *  空闲/繁忙按 DeepSeek 实际时段划分：工作日繁忙 09:00–12:00、14:00–18:00，
     *  即 o'clock 9/10/11 与 2/3/4/5（忙）；空闲为 12/1 与 6/7/8（闲，含 18:00）。
     *  w/busyColor/idleColor 由用户在设置中自定义。 */
    function buildClockRing(ringEl, isWeekend, w, busyColor, idleColor) {
      ringEl.textContent = ''
      if (isWeekend) {
      ...
      base.setAttribute('r', '90')
      base.setAttribute('class', 'dsg-ring-base')
      base.style.stroke = busyColor
      base.style.strokeWidth = w + 'px'
      ringEl.appendChild(base)
      // 空闲色弧：o'clock 12+1 → 345°..45°；6+7+8 → 165°..255°（18:00 属空闲）。
      const arcs = [
        arcPath(100, 100, 90, 345, 405),
        arcPath(100, 100, 90, 165, 255),
      ]
      for (const d of arcs) {
        const p = document.createElementNS('http://www.w3.org/2000/svg', 'path')
        p.setAttribute('d', d)
        p.setAttribute('class', 'dsg-ring-arc')
        p.style.stroke = idleColor
    ...
    function renderOdometer(odoEl, balance, threshold, maxBalance) {
      ...
      const l0 = svgEl('text', { class: 'dsg-odo-label', x: '44', y: '103', 'text-anchor': 'start' })
      l0.textContent = '¥0'
      const l1 = svgEl('text', { class: 'dsg-odo-label', x: '156', y: '103', 'text-anchor': 'end' })
      l1.textContent = fmtMoney(scale)
      const thrPos = polar(100, 100, 54, odoAngle(fthr))
    ...
    function renderPill(pillG, text) {
      pillG.textContent = ''
      const rect = svgEl('rect', { class: 'dsg-pill', x: '72', y: '45', width: '56', height: '14', rx: '7' })
      const t = svgEl('text', { class: 'dsg-pill-text', x: '100', y: '52' })
      t.textContent = text
...
`
        ...
        function render(data) {
          ...
          if (data.schedule) {
            ...
            const key = `${isWeekend}|${ringW}|${ringBusy}|${ringIdle}`
            if (key !== lastRingKey) {
              lastRingKey = key
              buildClockRing(ringEl, isWeekend, ringW, ringBusy, ringIdle)
            }
          }
