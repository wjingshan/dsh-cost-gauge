...
`
    ...
    function renderOdometer(odoEl, balance, threshold, maxBalance) {
      ...
      // 刻度：左(¥0)、右(满刻度)、顶、以及阈值位置。
      const ticks = [270, 360, 90, odoAngle(fthr)]
      for (const t of ticks) {
        const [ax, ay] = polar(100, 100, ODO_R + 8, t)
        const [bx, by] = polar(100, 100, ODO_R + 12, t)
        odoEl.appendChild(svgEl('line', { class: 'dsg-odo-tick', x1: ax.toFixed(2), y1: ay.toFixed(2), x2: bx.toFixed(2), y2: by.toFixed(2) }))
      }
      // 金额标签放在里程表内侧（弧内，不压外圈时间环）。
      const l0 = svgEl('text', { class: 'dsg-odo-label', x: '44', y: '90', 'text-anchor': 'start' })
      l0.textContent = '¥0'
      const l1 = svgEl('text', { class: 'dsg-odo-label', x: '156', y: '90', 'text-anchor': 'end' })
