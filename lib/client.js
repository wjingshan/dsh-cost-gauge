...
.dsg-ring-base{fill:none;stroke-width:15}
.dsg-ring-arc{fill:none;stroke-width:15;stroke-linecap:round}
...
`
    ...
    function buildClockRing(ringEl, isWeekend) {
      ...
      base.setAttribute('r', '94')
      base.setAttribute('class', 'dsg-ring-base')
      base.setAttribute('stroke', COL.busyRing)
      ringEl.appendChild(base)
      // 绿色空闲弧（o'clock 12+1 → 345°..45°；7+8 → 195°..255°），圆头端点与黄底圆角交接。
      const arcs = [
        arcPath(100, 100, 94, 345, 405),
        arcPath(100, 100, 94, 195, 255),
      ]
      for (const d of arcs) {
    ...
    function renderOdometer(odoEl, balance, threshold, maxBalance) {
      ...
      const thrPos = polar(100, 100, 56, odoAngle(fthr))
