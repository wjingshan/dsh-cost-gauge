...
@media (prefers-color-scheme: light){
  .dsg-root{--dsg-bg:#ffffff;--dsg-fg:#161616;--dsg-muted:#6d6d6d;
    ...
    --dsg-disc:transparent;
    --dsg-accent:#2f6fb3;box-shadow:none}
  /* 浅色主题去掉所有阴影效果 */
  .dsg-ring,.dsg-ring-arc,.dsg-pill{filter:none}
  .dsg-flapboard,.dsg-model-pill{box-shadow:none}
}
...
.dsg-idle .dsg-lamp.dsg-lamp-idle,.dsg-busy .dsg-lamp.dsg-lamp-busy{opacity:1}
.dsg-lamp-dot{transition:opacity .3s}
.dsg-idle .dsg-lamp.dsg-lamp-idle .dsg-lamp-dot,
.dsg-busy .dsg-lamp.dsg-lamp-busy .dsg-lamp-dot{filter:drop-shadow(0 0 4px currentColor)}
/* 模型解题中：激活状态灯一循环≈1.7s（亮≈0.6s、暗≈1.1s）；闲置时常亮不闪 */
.dsg-working.dsg-idle .dsg-lamp.dsg-lamp-idle .dsg-lamp-dot,
.dsg-working.dsg-busy .dsg-lamp.dsg-lamp-busy .dsg-lamp-dot,
.dsg-working .dsg-min-lamp,
.dsg-working .dsg-bar-lamp{animation:dsg-lamp-blink 1.7s linear infinite}
@keyframes dsg-lamp-blink{0%,35%{opacity:1}42%,100%{opacity:.15}}
...
`
    ...
    function setFlapChar(cell, ch) {
      ...
      nw.className = 'dsg-flap-char'
      nw.textContent = ch
      cell.appendChild(nw)
      cell.dataset.ch = ch
      if (old) {
        old.classList.remove('dsg-flap-in')
      ...
      nw.classList.add('dsg-flap-in')
      const clr = () => nw.classList.remove('dsg-flap-in')
    ...
    function startStars(layer, isWeekend, isPM, count = 60) {
      ...
      for (let i = 0; i < count; i++) {
        const el = svgEl('circle', { class: 'dsg-star', r: (0.45 + Math.random() * 0.8).toFixed(2), fill: colors[Math.floor(Math.random() * colors.length)] })
    ...
    function buildLampSvg(color, lit, progressRes) {
      ...
      svg.appendChild(svgEl('circle', { cx: '11', cy: '11', r: '9', fill: color, opacity: lit ? '0.28' : '0.12', class: 'dsg-lamp-base' }))
      const ring = svgEl('circle', { cx: '11', cy: '11', r: '8', fill: 'none', stroke: color, 'stroke-width': '2', 'stroke-linecap': 'round', 'stroke-dasharray': dash, transform: 'rotate(-90 11 11)', class: 'dsg-lamp-ring' })
      const dot = svgEl('circle', { cx: '11', cy: '11', r: '3.6', fill: color, class: 'dsg-lamp-dot' })
...
<div class="dsg-compact">
  ...
  <div class="dsg-row"><span>余额</span><span class="dsg-val dsg-balance2">—</span></div>
          ...
          function setModelText(text) {
            ...
            if (old) {
              old.classList.remove('dsg-model-in')
              old.classList.add('dsg-model-out')
              const rm = () => { if (old.isConnected) old.remove() }
              old.addEventListener('animationend', rm, { once: true })
              setTimeout(rm, 300)
            }
            nw.classList.add('dsg-model-in')
            const clr = () => nw.classList.remove('dsg-model-in')
            nw.addEventListener('animationend', clr, { once: true })
            setTimeout(clr, 350)
            // 防御：只清理多余的翻牌层（不影响撑宽的 measure）
            while (modelEl.querySelectorAll('.dsg-model-inner').length > 2) {
              const e = modelEl.querySelector('.dsg-model-inner')
              if (e) e.remove()
            }
          }
          function renderRing() {
          ...
          function renderLamps(peak, rate) {
            ...
            const C = 2 * Math.PI * 8
            const idleRing = idlePh.querySelector('.dsg-lamp-ring')
            const busyRing = busyPh.querySelector('.dsg-lamp-ring')
          ...
          const skinApi = {
            render(data) {
              ...
              el.classList.toggle('dsg-working', st.isWorking())
