/**
 * dsh-cost-gauge 宿主侧记账模块 —— 回放会话日志，按「事件时刻的费率 + 当时的模型」逐笔计价。
 *
...
/** 数据目录：~/.dsh/cost-gauge/ */
export const LEDGER_DIR = path.join(os.homedir(), '.dsh', 'cost-gauge')
...
/**
 * 会话账目重建：会话日志被压缩/重写后 seq 空间会变短，旧 cursor 会永久越过日志末尾
 * （症状：会话花费长时间不动）。清空该会话的游标、累计与每日桶，
 * 下一次回放会从当前日志的 seq 0 重新记账，保证总额与游标都回到有效范围。
 */
export function rebuildSession(ledger, sessionId) {
  ...
  if (ledger.days && ledger.days[sessionId] !== undefined) delete ledger.days[sessionId]
  return true
}
