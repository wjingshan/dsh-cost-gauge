...
- 🕐 **时钟造型**：24 小时表盘，仅一根时针指向当前北京时间；外圈用两种颜色表示全天费率——**绿色 = 空闲（标准）**、**黄色 = 繁忙（高峰）**。
- 🖐️ **可拖拽 / 可缩放**：按住标题栏拖动位置；拖拽右下角把手放大或缩小窗口（时钟随之缩放），位置与大小自动记忆。
- 🔍 **展开 / 缩小两种状态**：
  - 展开态：时钟 + 会话花费 + 余额 + 距下次切换倒计时；
  - 缩小态：只显示**话费、余额**，加**两盏状态灯**——🟡 黄=繁忙、🟢 绿=空闲，当前状态亮起、另一盏暗掉。
