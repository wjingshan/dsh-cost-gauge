...
- 📱 **窗口自适应**：窗口变窄（视口 ≤ 779px，即 < 780px）时自动切为 **48px 竖版最小化**（灯 + 花费 + 余额 + 模型徽标），点击展开；窗口恢复（≥ 860px）自动还原。
...
> .\install-dsh-cost-gauge.ps1 -Ref v1.3.1      # 锁指定版本
> ```
...
### 手动安装（示例锁 v1.3.1，可换成任意 Release tag）
